  ...
  useEffect(() => {
    ...
    async function initWorkspace() {
      ...
      const { data: proj1 } = await client.from("projects").insert([
        { user_id: user.id, title: "Website Redesign", workspace_id: wsId }
      ]).select("id").single();
      const { data: proj2 } = await client.from("projects").insert([
        { user_id: user.id, title: "Mobile App", workspace_id: wsId }
      ]).select("id").single();
      ...
      const tasksToInsert = [
        ...
        { title: "Write unit tests", status: "To do", projectId: proj2.id, assignedTo: user.id }
      ];
      await client.from("tasks").insert(tasksToInsert);

      // 6. Create sample activity logs
      await client.from("activity_logs").insert([
        { user_id: user.id, description: "You joined the workspace", workspace_id: wsId },
